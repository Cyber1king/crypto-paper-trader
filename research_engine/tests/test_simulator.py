...
def signal(side: str = "long") -> Signal:
    return Signal(
        datetime.now(timezone.utc), side, "test", 100, 90, 110, "up", True, False
    )


def test_paper_broker_records_profitable_long() -> None:
    broker = PaperBroker(10_000)
    broker.open_from_signal(signal(), risk_fraction=0.1)
    trade = broker.close(110, datetime.now(timezone.utc))
